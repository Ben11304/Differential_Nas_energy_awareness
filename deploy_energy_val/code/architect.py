...
class Architect(object):
    def __init__(self, model, args, device='cuda'):
        ...
        self.acc_weight = getattr(args, 'acc_weight', 0.9)
        self.energy_weight = getattr(args, 'energy_weight', 0.05)
    ...
    def _backward_step(self, input_valid, target_valid, ):
        ...
        self.max_seen_energy = torch.max(self.max_seen_energy, loss_energy.detach())
        self.max_seen_loss = torch.max(self.max_seen_loss, loss_task.detach())
        energy_normalized = loss_energy / self.max_seen_energy
        loss_task_normalized = loss_task / self.max_seen_loss

        loss = self.acc_weight * loss_task_normalized + self.energy_weight * energy_normalized
        loss.backward()
