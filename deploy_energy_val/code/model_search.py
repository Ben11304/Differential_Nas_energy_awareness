...
class MixedOp(nn.Module):
    def __init__(self, C, stride, primitives):
        ...
        self._energy_flags = []
        self._energy = []
        # Per-sample MACs per op, captured once after the ops self-measure on
        # their first forward (Paper B FLOPs-swap). None until first batch.
        self._macs_per_sample = None
    ...
    def forward(self, x, weights):
        # Paper B swap (estimated energy -> exact MACs):
        # cost = sum_i softmax(alpha)_i * MACs_i, MACs_i exact per-op constants
        # (ptflops conv-exact; see _audit_macs.py). Differentiable w.r.t. alphas
        # through the weights, same lookup pattern as before. The old sklearn
        # energy estimate is still computed and cached on each op (op.energy) —
        # kept available for the estimated-energy ablation arm (swap attr back).
        result = sum(w * op(x) for w, op in zip(weights, self._ops))
        if self._macs_per_sample is None:
            # ops have now self-measured (first forward); normalize per sample
            # using THIS batch's size — the same batch the measurement used.
            self._macs_per_sample = [
                float(getattr(op, "MACs", 0)) / x.size(0) for op in self._ops]
        total_cost = sum(w * m for w, m in zip(weights, self._macs_per_sample))
...
class Network(nn.Module):
    def __init__(self, C, num_classes, primitive_sets, criterion,
        ...
        # NOTE: original reduction logic produces spatial-size mismatches in Cell.forward
        # (strides are applied inconsistently when states list is initialised with only s1).
        # For classification sweeps on DFU we disable per-cell reduction and rely on
        # the adaptive-pool head for the final pooling.
