...
# count_module_macs: single source of truth lives in utils.py (functionally
# identical duplicate removed here 2026-06-12 — Paper B FLOPs-swap step 2).
# NOTE: energy() below wraps each layer in nn.Sequential before counting, so
# the MaxPool2d manual branch in count_module_macs never fires from this path;
# pooling is counted by ptflops' elementwise convention (2*numel, stride-
# invariant). Convs are counted exactly (verified by _audit_macs.py).
...
class SkipConnect(nn.Module):
    def __init__(self, C, stride, affine=True):
        ...
        if stride == 1:
            self.op = Identity()
            self.MACs = 0  # Identity = no compute (was hardcoded 5 — audit fix)
        else:
            self.op = FactorizedReduce(C, C, affine)
            self.C = C
            self.MACs = 0  # populated from FactorizedReduce on first forward
...
class MaxPoolMAC(nn.Module):
    def __init__(self, kernel_size=3, stride=1, padding=1, affine=False, C=1):
        ...
        self.MACs = 0  # populated via energy() on first forward (init was 5)
